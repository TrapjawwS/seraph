...
const rgb = (r, g, b) => `rgb(${r},${g},${b})`;
    ...
    const blend = {
        ...
        multiply: (a, b, opacity) => {
            ...
            b = b.normalize();
            return new Color(
                ((a.r * b.r * opacity) + (a.r * (1.0 - opacity))) * 255,
                ((a.g * b.g * opacity) + (a.g * (1.0 - opacity))) * 255,
                ((a.b * b.b * opacity) + (a.b * (1.0 - opacity))) * 255);
        },
        add: (a, b) => {
            a = a.normalize();
            b = b.normalize();
    ...
    class Rain extends GameState {
        ...
        start() {
            ...
            this.rainDrops.forEach((x, i) => { x.visible = true; x.hide = false; x.x = i * xSpread, x.y = Math.random() * yVariation });
        }
        stop() {
            this.raining = false;
        }
    ...
    class Zone extends GameState {
        ...
        addGrass() {
            ...
            for (let i = 0; i < grassRows; ++i) {
                for (let j = 0; j < grassColumns; ++j) {
                    if (Math.random() < 0.05) {
                        this.grass.push({ x: j * grassWidth, y: i * grassHeight });
                    }
                }
            }
        }
            ...
            for (let i = 0; i < rows; ++i) {
                for (let j = 0; j < columns; ++j) {
                    if (Math.random() < 0.005) {
                        this.flowers.push({ x: j * width, y: i * height, state: 0 });
                    }
                }
            }
        }
        ...
        rollWraiths() {
            this.wraiths = [];
            this.hasWraith = Math.random() <= 0.4 && !this.isDestination && !this.isStartingPoint
                && this.game.state.zone != this;
            if (this.hasWraith) {
    ...
    class Gameplay extends GameState {
        constructor(game, renderer) {
            ...
            if (!this.SoundLayer.BGM.playing) {
                this.SoundLayer.BGM.play({ loop: true })
            }
            // END: Testing Sounds
        ...
        update(elapsed) {
            ...
            if (this.canMove) {
                if (this.keyboard.isKeyDown("shiftleft") || this.keyboard.isKeyDown("shiftright")
                    && !this.game.holdingBreath) {
    ...
    class Game {
        constructor() {
            ...
            this.stateIndex = -1;
            this.states = [
                () => new Intro(this, this.renderer),
                () => new Gameplay(this, this.renderer),
    ...
    function addEvent(signature, ...args) { window.on(signature, ...args) }
    function removeEvent(signature, ...args) { window.off(signature, ...args) }
    ...
    `
        ...
        LungCapacityComponent() {
            ...
            const OxygenBars = Array.from({ length: game.lungCapacity }).map(bar => `
